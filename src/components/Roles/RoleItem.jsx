...
export default function RoleItem({ role, onDelete, onUpdate }) {
  console.log(role);
  ...
  const handleUpdate = () => {
    onUpdate(role?.id, { roleName: title }, handleOpenModal);
  };
  return (
    <>
      <tr class="text-gray-700 dark:text-gray-400">
        <td class="p-4 text-sm">{role?.name}</td>
        <td class="p-4">
      ...
      {isOpenDelete && (
        <div className="fixed inset-0 z-[99999] flex items-end bg-black bg-opacity-50 sm:items-center sm:justify-center">
          <div class="px-4 py-6 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800 w-[600px]">
            ...
            <div className="">Bạn có chắc chắn muốn xóa role này không?</div>
            <div className="flex justify-end">
              <button
