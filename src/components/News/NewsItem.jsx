...
export default function NewsItem({ news, onDelete, onUpdate }) {
  ...
  const handleUpdate = () => {
    onUpdate(news?.id, { title, description }, handleOpenModal);
  };
  return (
    <>
      <tr class="text-gray-700 dark:text-gray-400">
        <td class="p-4 text-sm">
          <div className="text-sm line-clamp-4">{news?.title}</div>
        </td>
        <td class="p-4 text-sm">
        ...
        <td class="p-4 text-sm">
          <div className="text-sm line-clamp-4">{news?.updateAt}</div>
        </td>
      ...
      {isOpenDelete && (
        <div className="fixed inset-0 z-[99999] flex items-end bg-black bg-opacity-50 sm:items-center sm:justify-center">
          <div class="px-4 py-6 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800 w-[600px]">
            ...
            <div className="">Bạn có chắc chắn muốn xóa tin tức này không?</div>
            <div className="flex justify-end">
              <button
